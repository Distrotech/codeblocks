...
if [ "x$REV" != "x$OLD_REV" -o ! -r $REV_FILE ]; then
	...
	echo "m4_define([SVN_REVISION], 15.12svn$REV)" >> $REV_FILE
	...
	if [ "x$DCH" != "x" ]; then
		if [ -x "$DCH" ]; then
			...
			if [ $REV -gt $AKT_REV ]; then
				dch -v 15.12svn$REV "New svn revision"
			fi
		fi
	else
		mv debian/changelog debian/changelog.tmp
		sed "1 s/(15.12svn[^-)]*/(15.12svn$REV/" < debian/changelog.tmp > debian/changelog
